...
class SMTPNotifier(Notifier):
    ...
    def _create_email_body(self, summary: ExecutionSummary, level: NotificationLevel) -> str:
        ...
        emoji, color, status_text = status_info.get(
            level, ("ℹ️", "blue", "Info")
        )
...
class WebhookNotifier(Notifier):
    ...
    def _create_slack_payload(
        self, summary: ExecutionSummary, level: NotificationLevel
    ) -> dict:
        ...
        if summary.log_url:
            fields.append({"title": "Logs", "value": f"<{summary.log_url}|View Logs>", "short": False})
